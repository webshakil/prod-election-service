    ...
    try {
      ...
      if (result.rows[0]) {
        console.log('✅ Found active subscription:', {
          ...
          processing_fee_percentage: result.rows[0].processing_fee_percentage,
          processing_fee_enabled: result.rows[0].processing_fee_enabled,
          processing_fee_type: result.rows[0].processing_fee_type,
    ...
    try {
      ...
      const tableCheckQuery = `
        SELECT EXISTS (
          ...
          WHERE table_name = 'votteryyy_elections'
        );
      `;
  ...
  async checkEligibility(userId, isSubscribed = false) {
    try {
      ...
      if (subscription) {
        ...
        // 🔧 FIXED: Determine processing fee - check percentage first regardless of enabled flag
        let processingFee = 0;
        
        // Try percentage fee first (most common)
        if (subscription.processing_fee_percentage) {
          processingFee = parseFloat(subscription.processing_fee_percentage);
          console.log('💰 Using percentage fee:', processingFee);
        } 
        // Then try fixed fee
        else if (subscription.processing_fee_fixed_amount) {
          processingFee = parseFloat(subscription.processing_fee_fixed_amount);
          console.log('💰 Using fixed fee:', processingFee);
        }
        
        console.log('💰 Final processing fee:', processingFee);

        return {
          canCreate,
          subscriptionType: subscription.plan_type,
          planName: subscription.plan_name,
          billingCycle: subscription.billing_cycle || (subscription.payment_type === 'pay_as_you_go' ? 'Usage-based' : 'N/A'),
          amount: subscription.paid_amount || subscription.price || 0,
...
//last workable codes with pay as you go processing fee
// import pool from '../config/database.js';
// import { FREE_TIER_LIMITS } from '../config/constants.js';
